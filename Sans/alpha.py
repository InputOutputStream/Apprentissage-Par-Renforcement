...
class Array:
    ...
    def __setitem__(self, key, value):
        if isinstance(key, int):
            ...
            for i in range(self.shape[0]):
                for j in range(self.shape[1]):
                    if key.data[i][j] == 0 or key.data[i][j] == 1 :
    ...
    def _broadcast_to_shape(arr, shape):
        ...
        try:
            if len(arr.shape) == 1 and len(shape) == 2:
                if arr.shape[0] == shape[1]:
                    return Array([arr.data] * shape[0])
                elif arr.shape[0] == 1:
                    stretched = Array([arr.data * shape[1]])
                    return Array(stretched.data * shape[0])

            elif len(arr.shape) == 2 and len(shape) == 2:
                if arr.shape[0] == 1 and arr.shape[1] == shape[1]:
                    return Array(arr.data * shape[0])
                elif arr.shape[1] == 1 and arr.shape[0] == shape[0]:
                    return Array([row * shape[1] for row in arr.data])
                elif arr.shape[0] == 1 and arr.shape[1] == 1:
                    return Array([[arr.data[0][0]] * shape[1]] * shape[0])

            elif len(arr.shape) == 0:
                return Array([[arr.data] * shape[1]] * shape[0])
    ...
    def _unary_elementwise_op(self, op):
        if len(self.shape) == 1:
            ...
            return Array._unary_elementwise_op(self[0], op)
    ...
    def maximum(arr1, arr2):
        arr1, arr2 = Array._broadcast(arr1, arr2)
        result = arr1 > arr2
        return result
    
    @staticmethod
    def minimum(arr1, arr2):
        return -Array.maximum(-arr1, -arr2)
    ...
    def _stretch(self, axis, n_copies):
        ...
        if axis == 0:
            temp = [row for row in temp] * n_copies
        elif axis == 1:
            if len(self.shape) == 1:
                temp = [[elem] for elem in temp] * n_copies
            else:
    ...
    def array_equal(arr1, arr2):
        ...
        return _array_equal(arr11, arr22)
    ...
    def squeeze(self, axis=None):
        ...
        if axis is None:
            new_shape = [dim for dim in new_shape if dim != 1]
        else:
            new_shape.pop(axis)
    ...
    def mean(self, axis=None):
        def calculate_mean(data):
            total_sum = sum(data)
            return total_sum / len(data) if len(data) > 0 else 0

        if axis is None:
            flat_data = self.flatten()
            return calculate_mean(flat_data)
        
        if axis < 0 or axis >= len(self.shape):
            raise ValueError(f"Axis {axis} is out of bounds for array of shape {self.shape}")
        
        if len(self.shape) == 1:
            return self.sum() / self.size

        if axis == 0:
            transposed_data = self.T
        else:
            transposed_data = [list(row) for row in self.data]

        means = []
        for sublist in transposed_data:
            # print(sublist)
            means.append(calculate_mean(Array(list(sublist)).flatten()))
